...
原件不会按“能否直接抽出文字”分成两套精度不同的正文。`pdftotext` 只做快速预检和隔离判断；PDF、扫描图片和 Office 转换结果都进入同一套完整文档解析流水线。这样会多消耗一些本地 GPU 时间，但不会因为数字 PDF 恰好带文本层，就丢掉标题层级、表格结构和阅读顺序。
...
*图：PDF 与图片统一由 PaddleOCR-VL 完整流水线恢复文字、版面和跨页结构，再进入后续脱敏与 Markdown 归一化。*
...
因此归一化最终成为 crawler 仓库里由维护者工作站显式运行的 `task crawler:ocr:run`，而不是生产 crawler 主循环或服务器常驻服务的一部分。RTX 5070 本地使用 CUDA 12.9 的 PaddlePaddle GPU 包；原件和 Canonical Markdown 分别从私有 Forgejo 仓库拉取、抽查后推送。
