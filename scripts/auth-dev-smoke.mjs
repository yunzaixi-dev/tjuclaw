...
// Exercises the actual running `task dev` proxy. Captured Mailpit login is
// opt-in via AUTH_DEV_MAIL_MODE=captured; the default real SMTP shares the
// production courier and is not drained here.
const directory = new URL('../test-results/auth-dev/', import.meta.url);
await mkdir(directory, { recursive: true, mode: 0o700 });
const captured = process.env.AUTH_DEV_MAIL_MODE === 'captured';
...
try {
  ...
  if (!captured) {
    ...
    assert.ok(code, 'Captured Kratos email contains an OTP');
