...
- `https://draw.tjuclaw.cloud` — static tldraw board (local persistence only; no product auth)
...
| `draw/` | Static tldraw board for EdgeOne (`draw.tjuclaw.cloud`); own lockfile |
...
| `task draw:dev` / `task draw:build` | tldraw board on `:5175`; static `draw/dist` for EdgeOne |
