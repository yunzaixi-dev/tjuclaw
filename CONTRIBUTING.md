...
- 根目录自有的文档、Taskfile、CI、运维与组合工具采用 **Apache-2.0**；范围说明见 `NOTICE`。
- `frontend/`（`tjuclaw-client`）和 `cli/`（`tjucli`）采用 **GPL-3.0-only**，修改或再发布时必须保留 GPLv3 条款与对应源码要求。
- `backend/`（`tjuclaw-server`）和 `crawler/`（`tjuclaw-crawler`）为闭源组件，保留全部权利，除非另有书面协议，不得复制、修改、分发或制作衍生作品。
- 根目录许可证不会覆盖 Git 子模块内容；第三方依赖、字体、图标和素材继续遵循各自许可证。

修改某个子模块时，必须在该子模块仓库中提交并发布对应许可证文件，再在集成仓更新 submodule 指针。不要在根仓通过额外声明把闭源组件误标为开源。
